...
def main():
    ...
    xstd = X_tr.std(axis=(0, 1), keepdims=True)
    print(f"  X_mean range: [{xmean.min():.4f}, {xmean.max():.4f}]")
    print(f"  X_std range: [{xstd.min():.4f}, {xstd.max():.4f}]")
    X_tr, X_va, X_te, y_tr, y_va, y_te = normalize(xmean, xstd, X_tr, X_va, X_te, y_tr, y_va, y_te)
